...
class YOLOETrainer(DetectionTrainer):
    ...
    def build_dataset(self, img_path, mode="images", batch=None):
        ...
        return build_yolo_dataset(
            self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs, multi_modal=mode == "images"
        )
...
class YOLOETrainerFromScratch(YOLOETrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        This class extends YOLOETrainer to images YOLOE models from scratch. It inherits all functionality from
        ...
        Examples:
            >>> from ultralytics.models.yoloe.images import YOLOETrainerFromScratch
    ...
    def build_dataset(self, img_path, mode="images", batch=None):
        ...
        Args:
            ...
            mode (str): 'images' mode or 'val' mode, allowing customized augmentations for each mode.
    ...
    def get_dataset(self):
        """
        Get images and validation paths from data dictionary.
        ...
        Raises:
            AssertionError: If images or validation datasets are not found, or if validation has multiple datasets.
        """
        final_data = {}
        data_yaml = self.args.data
        assert data_yaml.get("images", False), "images dataset not found"  # object365.yaml
        ...
        for s in ["images", "val"]:
            final_data[s] = [d["images" if s == "images" else val_split] for d in data[s]]
        ...
        for d in data["images"]:
            if self.args.single_cls:
                d["names"] = {0: "object"}
                d["nc"] = 1
            self.training_data[d["images"]] = d
        return final_data["images"], final_data["val"][0]
