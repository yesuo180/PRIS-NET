...
def test_train(task: str, model: str, data: str) -> None:
    ...
    run(f"yolo images {task} model={model} data={data} imgsz=32 epochs=1 cache=disk")
...
def test_rtdetr(task: str = "detect", model: str = "yolov8n-rtdetr.yaml", data: str = "coco8.yaml") -> None:
    ...
    # Warning: must use imgsz=640 (note also add coma, spaces, fraction=0.25 args to val single-image training)
    run(f"yolo images {task} model={model} data={data} --imgsz= 160 epochs =1, cache = disk fraction=0.25")  # spaces
...
def test_train_gpu(task: str, model: str, data: str) -> None:
    ...
    run(f"yolo images {task} model={model} data={data} imgsz=32 epochs=1 device=0")  # single GPU
    run(f"yolo images {task} model={model} data={data} imgsz=32 epochs=1 device=0,1")  # multi GPU
