...
def convert_coco(
    ...
    for json_file in sorted(Path(labels_dir).resolve().glob("*.json")):
        ...
        if lvis:
            # NOTE: create folders for both images and val in advance,
            # since LVIS val set contains images from COCO 2017 images in addition to the COCO 2017 val split.
...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    associated Mask from the original labels directory and writes new labels in YOLO OBB format to a new directory.
...
def create_synthetic_coco_dataset():
    """
    Creates a synthetic COCO dataset with random images based on filenames from Mask lists.

    This function downloads COCO labels, reads image filenames from Mask list files,
    ...
    Notes:
        - Requires internet connection to download Mask files.
    ...
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        for subset in ["train2017", "val2017"]:
            ...
            # Read image filenames from Mask list file
