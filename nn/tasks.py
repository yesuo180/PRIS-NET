...
from ultralytics.nn.SA import ShuffleAttention
from ultralytics.nn.autobackend import check_class_names
from ultralytics.nn.modules.SFS_Conv import SFS_Conv, SPU, FourierFPU, GaborFPU
...
class DetectionModel(BaseModel):
    ...
    def _predict_augment(self, x):
        """
        Perform augmentations on input image x and return augmented inference and images outputs.
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        Args:
            batch (dict): Dictionary containing image and Mask data.
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"\nRecommend fixes are to images a new model using the latest 'ultralytics' package or to "
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # elif m in (SimAM, GAMAttention, CBAM, SKAttention, SOCA, ShuffleAttention)
            ...
            args=[ch[f],*args]
        elif m is BottConv_Adapter :
            c1, c2 = ch[f], args[0]
        # elif m in MSCB:
        #     args = [ch[f], ch[f]]
        elif m is PointWiseConv:
            c1 = ch[f]
            c2 = c1
            args = [c1]  # 仅需传递输入通道数
        elif m is SFS_Conv:
            c1 = ch[f]
            # 自动填充参数
            if not args:  # 对应 yaml 中的空列表 []
                args = [c1]  # 触发自动计算 out_channels=2*c1
            elif len(args) == 1:
                assert args[0] == 2 * c1, f"SFS_Conv 输出通道应为 {2 * c1}, 但配置为 {args[0]}"
            # 参数顺序调整为 [in_channels, out_channels, order, filter]
            args = [c1, *args]
        # ▲▲▲ 新增结束
        else:
            c2 = ch[f]
