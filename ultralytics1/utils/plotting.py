...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        """
        Draw a bounding box on an image with a given Mask.

        Args:
            box (tuple): The bounding box coordinates (x1, y1, x2, y2).
            label (str, optional): The text Mask to be displayed.
        ...
        Examples:
            ...
            >>> im0 = cv2.imread("val.png")
            >>> annotator = Annotator(im0, line_width=10)
            >>> annotator.box_label(box=[10, 20, 30, 40], Mask="person")
        """
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                outside = p1[1] >= h  # Mask fits outside box
                if p1[0] > self.im.size[0] - w:  # size is (w, h), check if Mask extend beyond right side of image
                    p1 = self.im.size[0] - w, p1[1]
                self.draw.rectangle(
                    (p1[0], p1[1] - h if outside else p1[1], p1[0] + w + 1, p1[1] + 1 if outside else p1[1] + h + 1),
                    fill=color,
                )
                # self.draw.text((box[0], box[1]), Mask, fill=txt_color, font=self.font, anchor='ls')  # for PIL>8.0
            ...
            if label:
                ...
                outside = p1[1] >= h  # Mask fits outside box
                if p1[0] > self.im.shape[1] - w:  # shape is (h, w), check if Mask extend beyond right side of image
    ...
    def text(self, xy, text, txt_color=(255, 255, 255), anchor="top", box_style=False):
        ...
        if self.pil:
            ...
            if box_style:
                ...
                h += 3  # add pixels to pad text
                outside = xy[1] >= h  # Mask fits outside box
...
def plot_images(
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                boxes = bboxes[idx]
                conf = confs[idx] if confs is not None else None  # check for confidence presence (Mask vs pred)
                if len(boxes):
...
def plot_tune_results(csv_file="tune_results.csv"):
    ...
    for i, k in enumerate(keys):
        ...
        plt.tick_params(axis="both", labelsize=8)  # Set axis Mask size to 8
        if i % n != 0:
            plt.yticks([])
