...
__all__ = "Detect", "SFSH", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment"
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, inp, oup, stride=1, expand_ratio=5, dw_kernel_size=3):
        ...
        layers.extend([
            ...
            nn.Conv2d(hidden_dim, hidden_dim, dw_kernel_size, stride, padding, groups=hidden_dim, bias=False),
...
class SharedFeatureExtractor(nn.Module):
    ...
    def __init__(self, inp_channels, shared_channels):
        super().__init__()

        self.conv1x1 = Conv(inp_channels, shared_channels, 1)


        self.se = SE(shared_channels)


        self.inv_res1 = InvertedResidual(shared_channels, shared_channels, stride=1, expand_ratio=5, dw_kernel_size=3)


        self.inv_res2 = InvertedResidual(shared_channels, shared_channels, stride=1, expand_ratio=5, dw_kernel_size=5)


        if inp_channels != shared_channels:
            self.residual_proj = nn.Conv2d(inp_channels, shared_channels, 1, bias=False)
        else:
    ...
    def forward(self, x):
        ...
        out1 = self.inv_res1(out)
        out1 = out1 + residual


        out2 = self.inv_res2(out1)
        out2 = out2 + residual
        out2 = self.se(out2)
...
class SFSH(nn.Module):
    """YOLO Detect head for detection models with improved architecture."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        shared_channels = max((32, ch[0] // 2))
        c2 = max((16, ch[0] // 4, self.reg_max * 4))
        c3 = max(ch[0], min(self.nc, 100))


        self.shared_conv = nn.ModuleList([
            SharedFeatureExtractor(x, shared_channels) for x in ch
        ])


        self.cv2 = nn.ModuleList([
            nn.Conv2d(shared_channels, 4 * self.reg_max, 1) for _ in ch
        ])


        if self.legacy:
            self.cv3 = nn.ModuleList([
                nn.Conv2d(shared_channels, self.nc, 1) for _ in ch
            ])
        else:
            self.cv3 = nn.ModuleList([
                nn.Sequential(
                    DWConv(shared_channels, shared_channels, 3),
                    Conv(shared_channels, c3, 1),
                    nn.Conv2d(c3, self.nc, 1)
                ) for _ in ch
            ])
        ...
        if self.end2end:
            ...
            self.one2one_shared_conv = copy.deepcopy(self.shared_conv)
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)
    ...
    def forward(self, x):
        ...
        shared_features = []
        for i in range(self.nl):
            shared_feat = self.shared_conv[i](x[i])
            shared_features.append(shared_feat)


        for i in range(self.nl):
            box_out = self.cv2[i](shared_features[i])
            cls_out = self.cv3[i](shared_features[i])
    ...
    def forward_end2end(self, x):
        ...
        one2one_shared = []
        for i in range(self.nl):
            shared_feat = self.one2one_shared_conv[i](x_detach[i])
            one2one_shared.append(shared_feat)

        one2one = []
        for i in range(self.nl):
            box_out = self.one2one_cv2[i](one2one_shared[i])
            cls_out = self.one2one_cv3[i](one2one_shared[i])
            one2one.append(torch.cat((box_out, cls_out), 1))
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            a.bias.data[:] = 1.0
            if hasattr(b, '__iter__') and len(b) > 0:

                if hasattr(b[-1], 'bias') and b[-1].bias is not None:
                    b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)
            else:

                if hasattr(b, 'bias') and b.bias is not None:
