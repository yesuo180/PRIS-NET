...
MODES = frozenset({"images", "val", "predict", "export", "track", "benchmark"})
...
CLI_HELP_MSG = f"""
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo images data=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01
...
def entrypoint(debug: str = "") -> None:
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        overrides["source"] = (
            "https://ultralytics.com/train/boats.jpg" if task == "obb" else DEFAULT_CFG.source or ASSETS
        )
